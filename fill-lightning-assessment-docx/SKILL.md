...
5. 读取 `w:sym` Wingdings 复选框：字符 `00FE` 表示已勾选，`00A8` 表示未勾选，并将勾选符号与后续选项文字关联。
6. 查找同时包含“防雷安全管理”和“雷电灾害历史”的表格，将其识别为 A.4，收集所有已勾选的直接赋值条件。
7. 将附件1字段和 A.4 条件交给评分逻辑，再复制文件 A 并把计算结果写入文件 B。

## 数据处理规则

- 从文件 A 读取“附件1：企业基础信息收集表”。
- 从文件 A 读取表 A.4 中已经勾选的“雷电灾害事故发生可能性直接赋值‘3’”条件，不推测、不修改 A.4。
- 计算并填写附件2中的 A.1—A.3、A.5—A.8。
- 只填写附件2目标表中的空白结果单元格，保留已有非空值。
- 不修改附件1、表 A.4、封面、其他附件和无关表格。
- 除 `word/document.xml` 外，文件 B 中所有 DOCX 包成员必须与文件 A 逐字节一致；在 `document.xml` 中只允许修改已识别的附件2目标表。
