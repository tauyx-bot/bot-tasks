...
def calculate_assessment(fields: dict[str, Any], stem: str, a4_review: dict[str, Any]) -> dict[str, Any]:
    ...
    # Note 2 of A.1 makes A.4 an override of the weighted calculation: any
    # selected A.4 condition directly assigns the accident likelihood L to 3.
...
def main() -> int:
    ...
    for docx_path in docx_files:
        try:
            ...
            # Imported lazily to avoid a circular import through the shared
            # OOXML formatting helpers used by report_sections.
            from report_sections import extract_sections
            assessment["报告章节"] = extract_sections(docx_path, fields)
